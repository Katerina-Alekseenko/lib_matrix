...
START_TEST(sum_matrix) {
  ...
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < cols; j++) {
      m.matrix[i][j] = get_rand(DBL_MIN, 10000);
  ...
  ck_assert_int_eq(lib_sum_matrix(&m, &mtx, &res), OK);
...
Suite *suite_lib_sum_matrix(void) {
  Suite *s = suite_create("suite_lib_sum_matrix");
  TCase *tc = tcase_create("lib_sum_matrix");
